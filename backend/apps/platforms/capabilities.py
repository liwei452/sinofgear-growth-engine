...
def resolve_loaded_account_capabilities(account: SocialAccount) -> set[AccountCapability]:
    ...
    if account.provider == SocialAccount.Provider.BUFFER:
        ...
        if (
            ...
            or account.connection_state != SocialAccount.ConnectionState.CONNECTED
            or connection is None
            or connection.provider != connection.Provider.BUFFER
            or connection.organization_id != account.organization_id
            or connection.connection_state != connection.ConnectionState.CONNECTED
            or not connection.credential_reference.strip()
            or not account.provider_account_id.strip()
        ):
            return set()
        granted_codes = {AccountCapability.PUBLISH.value}
    else:
        if (
            credential is None

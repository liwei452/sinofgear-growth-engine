...
def run_publish_task(organization_id, task_id):
    ...
    failure = None
    try:
        post = execute_publish_task(task_id, organization_id=tenant_id)
...
def sync_post_metrics_hourly():
    ...
    def sync_one(organization_id, _remaining):
        # Current metrics implementation is deterministic demo data plus ORM only.
        with tenant_atomic(organization_id):
...
def reconcile_buffer_publish_task_job(organization_id, task_id):
    tenant_id = parse_tenant_organization_id(organization_id)
    task = reconcile_publish_task(task_id, organization_id=tenant_id)

...
## 13. DeepSeek V4 integration acceptance update (2026-08-12)

DeepSeek V4 Flash is now wired into the existing audited AI orchestration for ordinary content generation and Phase B1 lead analysis. Administrators can test, save, replace, and delete the organization-scoped credential from AI service settings; the key is stored through Windows Credential Manager in normal Windows operation and is never returned to the browser. Safe handling covers invalid credentials, insufficient balance, rate limiting, timeouts, schema errors, retry metadata, usage, cost ceilings, and audit reconciliation.

The free acceptance suite uses a strictly owned temporary E2E environment. Its credential adapter and HTTP transport are unavailable unless two generated ownership gates match; production settings fail closed. Browser acceptance covers settings success and recovery, permission denial, organization isolation, deletion, content generation, and lead analysis without contacting DeepSeek or making a paid request. The authoritative record is `docs/acceptance/deepseek-v4-provider.md`.

The one real paid smoke test remains **PENDING**. It must be run only while the user is present and explicitly approves the paid call. On a new computer, pull the repository, install the documented dependencies, start the application, and enter the DeepSeek key again through **高级设置 → AI 服务设置**; secrets are intentionally not copied through Git. Installer packaging, email delivery, and shutdown remain outside this acceptance commit and require the user's later approval after the real-provider result is reviewed.

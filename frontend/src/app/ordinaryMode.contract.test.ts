...
const rawFixtureStates = ["ANALYZED", "RUNNING", "SOURCE_IMPORT", "DRAFT", "READY", "ACTIVE", "HIGH"]
...
const lead = {
  id: "LeadCandidate-private-id", company_name: "北方传动", company_domain: "beifang.example",
...
const leadDetail = {
  ...
  evidence: [{
    id: "SourceSignal-private-id", source_signal_id: "SourceSignal-private-id", platform: "LinkedIn",
  ...
  latest_insight: {
    id: "LeadInsight-private-id", source_insight_id: null, origin: "AI", score: 88, score_band: "HIGH",
    ...
    extracted_requirement_values: {}, ai_audit: { run_id: "AIRun-private-id", ontology: "Ontology-private-id" },
...
const campaign = {
  id: "Campaign-private-id", name: "德国获客", description: "", status: "DRAFT", version: 1,
  product_ids: [], created_at: lead.created_at, updated_at: lead.updated_at,
}

const brief = {
  id: "ContentBrief-private-id", campaign_id: campaign.id, previous_version_id: null, version: 1,
  ...
  selling_points: ["精密磨齿"], advantages: ["交期稳定"], keywords: ["精密齿轮"], product_ids: ["product-1"],
  asset_ids: [], platform_ids: ["platform-1"], concept_links: [], created_by: 1, reviewed_by: 1,
  reviewed_at: lead.created_at, created_at: lead.created_at, updated_at: lead.updated_at,
}

const runningJob = {
  job_id: "AIRun-private-id", type: "SOURCE_IMPORT", status: "RUNNING", progress: 40,
...
function fixtureFetch(input: RequestInfo | URL): Promise<Response> {
  ...
  if (path.startsWith("/api/v1/master-contents")) return Promise.resolve(page([]))
  if (path.startsWith("/api/v1/products")) return Promise.resolve(page([{
    ...
    manufacturing_capabilities: [], inspection_capabilities: [], concept_links: [],
  }]))
  if (path === "/api/v1/platforms") return Promise.resolve(json({ results: [{ id: "platform-1", code: "LINKEDIN", name: "LinkedIn", capabilities: ["PUBLISH"] }] }))
  if (path.startsWith("/api/v1/assets")) return Promise.resolve(page([]))
  if (path.startsWith("/api/v1/knowledge/")) return Promise.resolve(page([]))
  if (path.startsWith("/api/v1/analytics")) return Promise.resolve(json({
    count: 1, total_clicks: 17, next: null, previous: null,
    results: [{ date: "2026-08-10", campaign_id: campaign.id, platform_id: "platform-1", country: "DE", product_id: "product-1", clicks: 17 }],
  }))
...
async function renderOrdinaryRoute(path: string) {
  ...
  const routeExpectations: Record<string, { heading: string; ready: string }> = {
    ...
    "/analytics": { heading: "效果", ready: "德国获客" },
    "/company-profile": { heading: "AI 对公司的了解", ready: "精密齿轮" },
  }
  const expected = routeExpectations[path]
  await screen.findByRole("heading", { name: expected.heading, level: 1 })
  await screen.findAllByText(expected.ready, { exact: false })
...
it.each(["/", "/promotion", "/lead-radar", "/analytics", "/company-profile"])(
  ...
  async (path) => {
    ...
    for (const state of rawFixtureStates) expect(view.container).not.toHaveTextContent(state)
...
it("keeps the ordinary shell shrinkable at 390 CSS pixels", async () => {

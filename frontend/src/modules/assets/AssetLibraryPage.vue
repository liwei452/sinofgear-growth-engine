...
<template>
  <main class="page-stack">
    <header class="workspace-head"><div><p class="eyebrow">可复用素材</p><h1>素材库</h1><p>集中管理图片、视频和文档，并查看它们关联的产品。</p></div><button v-if="canManage" class="primary-action" @click="dialog=true">上传素材</button></header>
    <p v-if="message" role="status">{{ message }}</p><section class="filters" aria-label="素材筛选"><label>类型<select v-model="type" @change="resetPage"><option value="">全部</option><option>IMAGE</option><option>VIDEO</option><option>DOCUMENT</option></select></label><label>状态<select v-model="status" @change="resetPage"><option value="">全部</option><option>READY</option><option>QUARANTINED</option></select></label><label>标签<input v-model="tag" placeholder="例如 product" @input="resetPage"></label><button v-if="type||status||tag" @click="clearFilters">清除筛选</button></section>
    <p v-if="assets.isPending.value" role="status">正在加载素材…</p><section v-else-if="assets.isError.value" role="alert" class="panel"><h2>素材没有加载成功</h2><p>{{ error(assets.error.value) }}</p><button @click="assets.refetch()">重新加载素材</button></section><section v-else-if="!assets.data.value?.results.length" class="panel"><h2>还没有符合条件的素材</h2><p>调整筛选条件，或上传第一份素材。</p></section><section v-else class="cards" aria-label="素材列表"><article v-for="asset in assets.data.value?.results" :key="asset.id" class="panel"><div class="card-head"><div><h2>{{ asset.original_filename }}</h2><p>{{ asset.asset_type }} · {{ asset.mime_type }} · {{ asset.size_bytes }} B</p></div><span class="pill">{{ asset.status }}</span></div><p v-if="asset.tags?.length">标签：{{ asset.tags.join('、') }}</p><p>关联产品：{{ asset.products?.map(p=>p.name_en).join('、')||'暂无' }}</p><div v-if="canManage" class="actions"><label>关联产品<select v-model="productByAsset[asset.id]"><option value="">请选择</option><option v-for="product in products.data.value" :key="product.id" :value="product.id">{{ product.name_zh||product.name_en }}</option></select></label><button :disabled="!productByAsset[asset.id]" @click="link.mutate({assetId:asset.id,productId:productByAsset[asset.id]})">关联</button></div><button @click="download(asset.id)">安全下载</button></article><nav class="actions" aria-label="素材分页"><button :disabled="!assets.data.value?.previous" @click="pageUrl=assets.data.value?.previous??null">上一页</button><button :disabled="!assets.data.value?.next" @click="pageUrl=assets.data.value?.next??null">下一页</button></nav></section>
    <OperationModal v-if="dialog" title="上传素材" title-id="upload-title" @close="closeUpload"><form class="modal" @submit.prevent="upload.mutate()"><label>文件<input ref="fileInput" required type="file" @change="pick"></label><label>素材类型<select v-model="uploadType"><option>IMAGE</option><option>VIDEO</option><option>DOCUMENT</option></select></label><label>语言<input v-model="language" placeholder="zh-CN"></label><label>标签（逗号分隔）<input v-model="tags"></label><p v-if="upload.isError.value" role="alert">{{ uploadError }}</p><div class="actions"><button type="button" @click="closeUpload">取消</button><button class="primary-action" :disabled="!file||upload.isPending.value">上传</button></div></form></OperationModal>
  </main>
</template>
<style scoped>.workspace-head,.card-head,.actions{display:flex;justify-content:space-between;gap:1rem;align-items:flex-start}.filters{display:grid;grid-template-columns:repeat(3,1fr);gap:1rem}.filters label,.modal label{display:grid;gap:.35rem}.cards{display:grid;gap:1rem}.panel,.filters{padding:1.1rem;border:1px solid var(--border-color,#d8dee8);border-radius:1rem;background:white}.pill{background:#eef3f8;padding:.25rem .6rem;border-radius:999px}.modal{display:grid;gap:1rem}.actions{justify-content:flex-end}@media(max-width:700px){.workspace-head{display:grid}.filters{grid-template-columns:1fr}}</style>

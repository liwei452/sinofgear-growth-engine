...
it("retries a failed parse without asking for another upload",async()=>{document.cookie="csrftoken=t; path=/";const failed={...result,job:{...result.job,status:"FAILED",error:{code:"parse_failed"}},facts:[]};const fetch=vi.fn().mockResolvedValueOnce(response(failed)).mockResolvedValueOnce(response(result));vi.stubGlobal("fetch",fetch);mount();await userEvent.click(screen.getByRole("button",{name:"查看已有事实"}));await userEvent.click(await screen.findByRole("button",{name:"重试解析"}));expect(await screen.findByText("Accuracy: DIN 6")).toBeInTheDocument();expect(fetch.mock.calls[1][0]).toBe("/api/v1/assets/a1/understanding/retry")})

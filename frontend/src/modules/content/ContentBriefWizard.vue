...
const missingProducts = computed(() => missingProductIds.value.map((id, index) => ({ id, label: `历史产品 ${index + 1}（名称暂不可用）` })))
const missingAssets = computed(() => missingAssetIds.value.map((id, index) => ({ id, label: `历史素材 ${index + 1}（名称暂不可用）` })))
const missingConcepts = computed(() => missingConceptIds.value.map((id, index) => ({ id, label: `历史知识 ${index + 1}（名称暂不可用）` })))
      ...
      <section ref="dialog" class="wizard-dialog" role="dialog" aria-modal="true" aria-labelledby="wizard-title">
        ...
        <template v-if="ordinaryExperience">
          ...
          <section v-else-if="step === 3" aria-labelledby="ordinary-material-step">
            ...
            <fieldset v-if="availableConcepts.length || unavailableConcepts.length || missingConceptIds.length || fieldErrors.concepts" data-field="concepts"><legend>已批准知识（可选）</legend><span v-if="fieldErrors.concepts" class="field-error">{{ fieldErrors.concepts }}</span><label v-for="item in availableConcepts" :key="item.id"><input v-model="conceptIds" type="checkbox" :value="item.id" :aria-label="`${item.label_en || item.label_zh} (${item.concept_type})`"> {{ item.label_zh || item.label_en }}</label><label v-for="item in unavailableConcepts" :key="`unavailable-${item.id}`"><input type="checkbox" checked :aria-label="`${item.label_en || item.label_zh} (${item.concept_type})（不可用，仅可移除）`" @change="removeConcept(item.id)"> {{ item.label_zh || item.label_en }} <small>不可用，仅可移除</small></label><label v-for="item in missingConcepts" :key="`missing-${item.id}`"><input type="checkbox" checked :aria-label="`${item.label}（不可用，仅可移除）`" @change="removeConcept(item.id)"> {{ item.label }} <small>不可用，仅可移除</small></label></fieldset>
          ...
          <section v-else aria-labelledby="ordinary-confirm-step"><h3 id="ordinary-confirm-step">确认方案</h3><dl><div><dt>产品</dt><dd>{{ productIds.map((id, index) => products.find(item => item.id === id)?.name_zh || `历史产品 ${index + 1}（名称暂不可用）`).join('、') }}</dd></div><div><dt>目标</dt><dd>{{ form.target_country }} · {{ form.customer_type }} · {{ form.content_objective }}</dd></div><div><dt>渠道与素材</dt><dd>{{ platformIds.length }} 个渠道 · {{ assetIds.length }} 份素材</dd></div><div><dt>方案重点</dt><dd>{{ form.selling_points }}；{{ form.advantages }}</dd></div></dl><p>确认后会保存真实推广方案，等待有权限的同事审核；此处不会假装已经调用 AI 模型。</p></section>
        </template>

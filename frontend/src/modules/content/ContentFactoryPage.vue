...
import { formatOrdinaryError, ordinaryJobProgress } from "../../shared/presentation/ordinary"
...
const conceptsQuery = useQuery({ queryKey: conceptQueryKey, queryFn: ({ signal }) => ordinaryCreation.value ? listApprovedBriefConcepts({ signal }) : listBriefConcepts({ signal }), enabled: computed(() => enabled.value && has("knowledge.read")) })
...
function ordinaryJobNotice(job: Job) {
  return ordinaryJobProgress(job)
}
...
watch(canReadKnowledge, (current, previous) => {
  if (!previous || current) return
  const protectedQueryKey = conceptQueryKey.value
  conceptPages.reset()
  wizardOpen.value = false
  editingBrief.value = null
  cancelAndClear(protectedQueryKey)
}, { flush: "sync" })
  ...
  <div class="page-stack content-factory" aria-labelledby="factory-title">
    <template v-if="ordinaryExperience">
      ...
      <ol class="guided-flow" aria-label="推广流程">
        ...
        <GuidedStepCard :number="5" title="生成内容" description="确认方案后，才会提交真实的内容生成任务。" :state="guidedState(5)"><p v-if="trackedJob" role="status">{{ trackedJob.next_retry_at ? ordinaryJobNotice(trackedJob).message : jobStatusLabel(trackedJob) }}<span v-if="activeJobStatuses.has(trackedJob.status) && !trackedJob.next_retry_at"> · {{ trackedJob.progress }}%</span></p><p v-if="trackedJob?.next_retry_at" class="muted">{{ ordinaryJobNotice(trackedJob).recovery }}</p><p v-if="trackedJob?.status === 'FAILED'" role="alert">{{ ordinaryJobNotice(trackedJob).message }} {{ ordinaryJobNotice(trackedJob).recovery }}</p><p v-if="jobError" role="alert">{{ jobError }} <button v-if="trackedGeneration" type="button" @click="beginPolling(trackedGeneration.jobId)">重新检查生成进度</button></p><p v-if="ordinaryRecoveryError" role="alert">{{ ordinaryRecoveryError }} <button type="button" @click="startOrdinaryRecovery">重新检查生成记录</button></p><button v-if="trackedJob?.status === 'FAILED' && has('jobs.manage')" type="button" @click="jobAction(trackedJob, 'retry')">再次尝试</button><button v-if="latestBrief?.status === 'READY' && has('content.manage')" class="primary-action" type="button" :disabled="Boolean(actionId) || generationSubmissionBlocked" @click="startGeneration(latestBrief)">生成推广内容</button><p v-else class="muted">需要内容管理权限才能生成。</p></GuidedStepCard>
...
<style scoped>
.content-factory{display:grid;gap:1.5rem}.library-header,.promotion-header,.card-heading,.card-actions{display:flex;justify-content:space-between;gap:1rem;align-items:flex-start}.promotion-header{padding:1.5rem;border-radius:1.25rem;background:var(--sg-brand-tint)}.promotion-header>div:first-child{max-width:680px}.promotion-action{display:flex;align-items:center;flex-wrap:wrap;gap:.75rem}.promotion-action p{margin:0}.guided-flow{display:grid;gap:.8rem;padding:0;margin:0}.readiness-grid,.summary-grid,.card-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem}.readiness-grid article,.summary-grid article,.workflow-card{padding:1rem;border:1px solid var(--sg-line);border-radius:1rem;background:var(--sg-surface)}.readiness-grid article,.summary-grid article{display:grid;gap:.25rem}.readiness-grid strong,.summary-grid strong{font-size:1.35rem}.readiness-grid small{color:var(--sg-muted)}.advanced-disclosure{justify-self:start}.status-chip{padding:.25rem .55rem;border-radius:999px;background:var(--sg-brand-tint);color:var(--sg-brand);font-weight:700}.notice,.form-alert{padding:.8rem 1rem;border-radius:.75rem}.notice{background:var(--sg-status-success-tint);color:var(--sg-status-success)}.form-alert{background:var(--sg-status-danger-tint);color:var(--sg-status-danger)}.muted{color:var(--sg-muted)}.success{color:var(--sg-status-success);font-weight:700}.card-actions{justify-content:flex-end;flex-wrap:wrap}.button-link{display:inline-flex;width:max-content;text-decoration:none}.dialog-backdrop{position:fixed;inset:0;z-index:40;display:grid;place-items:center;padding:1rem;background:rgba(20,31,45,.55)}.brief-editor{display:grid;gap:.8rem;width:min(560px,100%);max-height:calc(100vh - 2rem);overflow:auto;padding:1.5rem;border-radius:1rem;background:var(--sg-surface)}.brief-editor label{display:grid;gap:.35rem}@media(max-width:700px){.library-header,.promotion-header{display:grid}.card-actions{justify-content:stretch}.card-actions button{width:100%}.readiness-grid{grid-template-columns:1fr}}
.promotion-header{background-color:var(--sg-brand-tint)}
</style>

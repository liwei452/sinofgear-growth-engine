      ...
      <section ref="dialog" class="review-dialog" role="dialog" aria-modal="true" aria-labelledby="review-title">
        ...
        <template v-else>
          ...
          <details class="technical-details">
            <summary>技术与来源详情</summary>
            <p>来源需求 {{ kind === 'master' ? (item as MasterContent).brief_id : '由主内容生成' }}，任务与版本信息已保留供审计。</p>
...
<style scoped>
.dialog-backdrop{position:fixed;inset:0;z-index:40;display:grid;place-items:center;padding:1rem;background:rgba(20,31,45,.55)}.review-dialog{width:min(860px,100%);max-height:calc(100vh - 2rem);overflow:auto;padding:1.5rem;border-radius:1rem;background:#fff}.review-dialog header,.dialog-actions{display:flex;justify-content:space-between;gap:1rem}.content-fields,.edit-form,.reject-form,.audit-panel,.platform-picker{display:grid;gap:1rem}.body-copy{white-space:pre-wrap}.chips{display:flex;flex-wrap:wrap;gap:.5rem}.chips span{padding:.25rem .55rem;border-radius:999px;background:#edf4f1}.technical-details{margin-top:1rem;border:1px solid var(--sg-line);border-radius:.75rem;background:#f7f9fb;padding:.8rem 1rem}.technical-details>summary{cursor:pointer;color:var(--sg-muted);font-weight:800}.fact-evidence{display:grid;gap:.65rem;padding:1rem;border:1px solid #dce8f4;border-radius:.8rem;background:#f8fbff}.fact-evidence h3,.fact-evidence p{margin:0}.fact-evidence article{display:grid;gap:.35rem}.fact-evidence blockquote{margin:0;padding:.55rem .75rem;border-left:3px solid #8eb8e0;background:#fff;color:#41556a}.muted{color:#66788a}.dialog-actions{justify-content:flex-end;flex-wrap:wrap;margin-top:1rem}.notice,.form-alert{padding:.75rem;border-radius:.7rem}.notice{background:#edf8f2}.form-alert{background:#fff0ed;color:#79291d}.edit-form label,.reject-form label{display:grid;gap:.4rem}
</style>

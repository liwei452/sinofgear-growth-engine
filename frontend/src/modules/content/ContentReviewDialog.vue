    ...
    <div ref="backdrop" class="dialog-backdrop" @click.self="emit('close')">
      <section ref="dialog" class="review-dialog" role="dialog" aria-modal="true" aria-labelledby="review-title">
        ...
        <template v-else>
          ...
          <section v-if="kind === 'master'" class="audit-panel"><button type="button" @click="loadAudit">查看AI生成记录</button><div v-if="auditOpen && audit"><p><strong>{{ audit.model }}</strong> · {{ audit.provider }}</p><p>{{ audit.prompt.code }} · v{{ audit.prompt.version }}</p><p>置信度 {{ audit.confidence ?? '未提供' }} · {{ audit.started_at }} 至 {{ audit.finished_at || '进行中' }}</p><p>人工修订：{{ audit.human_correction ? '有' : '无' }}</p><details><summary>安全字段摘要</summary><p>输入字段：{{ safeFieldNames(audit.input_snapshot) }}</p><p>输出字段：{{ safeFieldNames(audit.output_json) }}</p></details></div></section>

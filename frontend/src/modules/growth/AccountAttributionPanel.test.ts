...
const workspace: GrowthWorkspace = {
  target_accounts: [
    { id: "account-pack", name: "PackTech GmbH", country: "Germany", industry: "Packaging machinery", employee_range: "51-200", website: "", is_demo: false, data_label: "Owned CRM record" },
    { id: "account-nord", name: "NordMotion AB", country: "Sweden", industry: "Automation equipment", employee_range: "51-200", website: "", is_demo: false, data_label: "Owned CRM record" },
  ],
  contacts: [],
  intent_signals: [{
    ...
    data_label: "Licensed public source", collection_method: "LICENSED_API", collection_method_label: "许可来源",
  ...
  reactivations: [
    {
      ...
      tier: "STRATEGIC", status: "APPROVED", is_demo: false, why_reactivate: "已有合法关系与近期证据",
    ...
    },
    {
      ...
      tier: "OBSERVATION", status: "SELECTED", is_demo: false, why_reactivate: "已有合法关系但证据不足",
...
it("shows only recorded funnel facts and explains unavailable outcome denominators", () => {
  ...
  expect(within(region).getByRole("button", { name: /候选 1/ })).toBeInTheDocument()
  expect(within(region).getByRole("button", { name: /人工核实 1/ })).toBeInTheDocument()
  expect(within(region).getByRole("button", { name: /资料补全 0/ })).toBeInTheDocument()
...
it("shows only licensed records and filters them by funnel stage", async () => {
  ...
  expect(within(region).queryByLabelText("数据性质")).not.toBeInTheDocument()
  expect(within(region).getByRole("article", { name: "Licensed Gears Ltd 候选记录" })).toBeInTheDocument()
  expect(within(region).queryByRole("article", { name: "Demo Motion Inc 候选记录" })).not.toBeInTheDocument()
...
it("keeps progressed records visible when drilling into an earlier funnel stage", async () => {
  ...
  await user.click(within(region).getByRole("button", { name: /候选 1/ }))
  expect(within(region).getByRole("article", { name: "Licensed Gears Ltd 候选记录" })).toBeInTheDocument()
  expect(within(region).queryByRole("article", { name: "Demo Motion Inc 候选记录" })).not.toBeInTheDocument()

...
type AttributionRow = { id: string; kind: "ACCOUNT" | "CANDIDATE"; name: string; country: string; isDemo: boolean; sources: string[]; channels: string[]; stage: StageKey | "ACCOUNT"; stageLabel: string; statusNote: string; evidence: string; eventTime: string; nextAction: string }

const props = defineProps<{ workspace: GrowthWorkspace }>()
const activeStage = ref<StageKey>("ALL")
const dataKind = ref<"ALL" | "DEMO" | "RECORDED">("ALL")
...
const candidateMap = computed(() => {
  ...
  for (const item of props.workspace.discovery?.candidates ?? []) result.set(item.id, item)
  for (const item of props.workspace.discovery?.enrichment_candidates ?? []) result.set(item.id, { ...result.get(item.id), ...item } as EnrichmentCandidate)
  return result
})
const followUpIds = computed(() => new Set(props.workspace.follow_ups.map(item => item.account_id)))
const draftIds = computed(() => new Set([...props.workspace.outreach_drafts.map(item => item.account_id), ...(props.workspace.reactivations ?? []).filter(item => item.draft).map(item => item.account_id)]))
const approvedIds = computed(() => new Set([...(props.workspace.reactivations ?? []).filter(item => item.status === "APPROVED").map(item => item.account_id), ...(props.workspace.crm_handoffs ?? []).map(item => item.account_id)]))
const evidenceIds = computed(() => new Set(props.workspace.intent_signals.map(item => item.account_id)))

function accountRow(account: TargetAccount): AttributionRow {
  const reactivation = (props.workspace.reactivations ?? []).find(item => item.account_id === account.id)
  const signal = props.workspace.intent_signals.find(item => item.account_id === account.id)
  ...
  const base = { id: account.id, kind: "ACCOUNT" as const, name: account.name, country: account.country, isDemo: account.is_demo, sources, channels }
...
const visibleRows = computed(() => rows.value.filter(item => (dataKind.value === "ALL" || (dataKind.value === "DEMO") === item.isDemo) && (market.value === "ALL" || item.country === market.value) && (source.value === "ALL" || item.sources.includes(source.value)) && (channel.value === "ALL" || item.channels.includes(channel.value)) && reachedStage(item, activeStage.value)))
...
const enrichedCount = computed(() => [...candidateMap.value.values()].filter(item => "latest_preview" in item && item.latest_preview).length)
const totalAccounts = computed(() => props.workspace.target_accounts.length)
...
const dataCostMicros = computed(() => props.workspace.field_provenance.reduce((sum, item) => sum + item.source_cost_micros, 0) + props.workspace.intent_signals.reduce((sum, item) => sum + (item.evidence_envelope?.source_cost_micros ?? 0), 0))
...
<template>
  <section class="growth-card account-attribution" role="region" aria-labelledby="account-attribution-title">
    ...
    <div class="attribution-segments"><span>Demo / Fake：{{ rows.filter(item => item.isDemo).length }} 条</span><span>许可 / 人工记录：{{ rows.filter(item => !item.isDemo).length }} 条</span></div>
    ...
    <div class="attribution-metrics" aria-label="核心指标">
      ...
      <article><span>数据成本 {{ dataCost }}</span><small>仅汇总 {{ workspace.field_provenance.length + workspace.intent_signals.filter(item => item.evidence_envelope).length }} 条已有成本记录</small></article>
    ...
    <form class="attribution-filters" @submit.prevent>
      <label>数据性质<select v-model="dataKind"><option value="ALL">全部</option><option value="DEMO">Demo / Fake</option><option value="RECORDED">许可 / 人工记录</option></select></label>
    ...
    <div v-if="visibleRows.length" class="attribution-records" aria-live="polite">
      <article v-for="item in visibleRows" :key="`${item.kind}-${item.id}`" :aria-label="`${item.name} ${item.kind === 'ACCOUNT' ? '归因记录' : '候选记录'}`">
        <div class="attribution-record-title"><div><small>{{ item.kind === "ACCOUNT" ? "目标账户" : "候选公司" }} · {{ item.country }}</small><h3>{{ item.name }}</h3></div><span :class="item.isDemo ? 'demo-badge' : 'verified'">{{ item.isDemo ? "Demo / Fake" : "许可 / 人工记录" }}</span></div>

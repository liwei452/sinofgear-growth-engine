...
const primaryMission = computed(() => missionsQuery.data.value?.[0] ?? null)
const steps = computed(() => {
  const mission = primaryMission.value
  ...
  const assetsReady = (assetsQuery.data.value?.results ?? []).some(asset => asset.status === "READY")
      ...
      : step.id === "market" && !mission
        ...
        : step.id === "icp" && !canReadProducts.value
          ...
          : step.id === "content" && !canReadAssets.value
            ...
            : step.summary
    if (step.id === "discovery" && mission) return { ...step, summary, route: `/missions/${mission.id}?view=customer` }
    if (step.id === "approval" && mission) return { ...step, summary, route: `/missions/${mission.id}` }
    return { ...step, summary }
  })
})
...
<template>
  <main class="promotion-page">
    ...
    <BusinessState
      v-if="companyFactsQuery.isPending.value || missionsQuery.isPending.value"
      kind="loading" title="正在读取推广记录" message="完成读取后才会确认当前步骤。"
    />
    <BusinessState
      v-else-if="companyFactsQuery.isError.value && isForbidden(companyFactsQuery.error.value)"
      kind="blocked" title="无权读取公司事实" message="无法确认公司资料是否已准备完成。"
    />
    <BusinessState
      v-else-if="companyFactsQuery.isError.value || missionsQuery.isError.value"
      kind="error" title="推广状态暂时无法读取" message="请稍后重试；未读取到的数据不会被标记为已完成。"
      action-label="重新加载" @action="companyFactsQuery.refetch(); missionsQuery.refetch()"
    />
    <BusinessState
      v-else-if="productsQuery.isError.value || assetsQuery.isError.value"
    ...
    <section class="journey" aria-labelledby="promotion-journey-title">

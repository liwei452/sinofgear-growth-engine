...
it("shows clean publishing prerequisites for an empty month",async()=>{const fetch=vi.fn((path:string)=>Promise.resolve(new Response(JSON.stringify(path.startsWith("/api/v1/publish-calendar")?{timezone:"UTC",start:"",end:"",metadata:{max_entries:100,returned_entries:0,truncated:false},days:[]}:path==="/api/v1/social-accounts"?{results:[]}:{next:null,previous:null,results:[]}),{status:200,headers:{"Content-Type":"application/json"}})));renderPublishing(["publishing.read","publishing.manage"],fetch);const emptyHeading=await screen.findByRole("heading",{name:"这个月还没有发布安排"});expect(emptyHeading).toBeInTheDocument();expect(screen.getByRole("navigation",{name:"内容与发布工作区"})).toBeInTheDocument();const emptyState=within(emptyHeading.closest("section")!);expect(emptyState.getByRole("link",{name:"审核内容"})).toHaveAttribute("href","/reviews");expect(emptyState.getByRole("link",{name:"连接渠道"})).toHaveAttribute("href","/platform-accounts");expect(emptyState.getByRole("link",{name:"下载发布包"})).toHaveAttribute("href","/promotion");expect(screen.queryByText(/发布成功/)).not.toBeInTheDocument()})
...
it("lets a publishing manager run one scheduled task now",async()=>{document.cookie="csrftoken=t; path=/";const entry={id:"run-task",platform_content_id:"content",social_account_id:"account",platform_id:"platform",status:"SCHEDULED",scheduled_at:"2026-08-10T08:00:00Z",requested_timezone:"UTC",attempt_number:0,retry_not_before:null,last_error:null,attempts:[],published_post:null};const fetch=vi.fn((path:string,init?:RequestInit)=>{if(path.startsWith("/api/v1/publish-calendar"))return Promise.resolve(new Response(JSON.stringify({timezone:"UTC",start:"",end:"",metadata:{max_entries:100,returned_entries:1,truncated:false},days:[{date:"2026-08-10",entries:[entry]}]}),{status:200,headers:{"Content-Type":"application/json"}}));if(path==="/api/v1/publish-tasks/run-task/run"&&init?.method==="POST")return Promise.resolve(new Response(JSON.stringify({...entry,status:"SUCCEEDED"}),{status:200,headers:{"Content-Type":"application/json"}}));if(path==="/api/v1/social-accounts")return Promise.resolve(new Response(JSON.stringify({results:[]}),{status:200,headers:{"Content-Type":"application/json"}}));return Promise.resolve(new Response(JSON.stringify({next:null,previous:null,results:[]}),{status:200,headers:{"Content-Type":"application/json"}}))});renderPublishing(["publishing.read","publishing.manage"],fetch);await userEvent.click(await screen.findByRole("button",{name:"立即运行"}));await waitFor(()=>expect(fetch.mock.calls.some(call=>call[0]==="/api/v1/publish-tasks/run-task/run")).toBe(true))})

...
import { missionAttributionQueryOptions } from "../attribution/api"
...
const attributionQuery = useQuery({
  ...missionAttributionQueryOptions(selectedMissionId.value),
...
<template>
  <main class="results-page">
    ...
    <template v-else-if="attribution">
      <section aria-labelledby="funnel-title" class="funnel-card"><div><h2 id="funnel-title">从机会到成交</h2><p>转化漏斗只使用当前任务的归因数据，不推测未记录的步骤。</p></div><ol class="funnel" aria-label="转化漏斗"><li v-for="step in funnelSteps" :key="step.label"><span class="step-label">{{ step.label }}</span><strong :class="{ unknown: observed(step.value) === '尚未记录' }">{{ observed(step.value) }}</strong><small>{{ step.detail }}</small></li></ol></section>
